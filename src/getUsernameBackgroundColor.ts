const COLOR_CACHE: Record<string, string> = {}; // Cache to store colors by username
 ...
 */
export const getUsernameBackgroundColor = (username: string): string => {
    if (!username) return '#888888'; // Neutral gray for empty usernames
 ...
 */
const ensureReadableColor = (color: string): string => {
 ...
 */
const lightenColor = (r: number, g: number, b: number): string => {
